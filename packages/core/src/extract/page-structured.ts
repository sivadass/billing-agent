...
/**
 * Reads the first `Product` offer out of the page's `application/ld+json`
 * blocks. Exported on its own because the workflow `json_ld` strategy must be
 * strictly schema.org — never the OG/selector guesses `extractFromPageHtml`
 * also tries.
 */
export function extractFromJsonLd(html: string): ExtractedPrice | null {
